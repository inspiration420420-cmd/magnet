...
let lastLevel=0,lastCores=-1,lastOver=0,lastChallenge=null,bossPulse=0,challengeScore=1,lastScore=0;
...
function applyChallenge(){if(typeof g==='undefined'||!g||g.over)return;const c=window.magnetChallengeActive;if(!c||c.level!==g.n)return;if(lastChallenge!==g.n){lastChallenge=g.n;challengeScore=c.modifier==='DOUBLE SCORE'?2:1;if(c.modifier==='RAPID')g.time=Math.max(8,g.time*.72);if(c.modifier==='HAZARD RUSH'){for(let i=0;i<Math.min(5,Math.max(2,Math.floor(g.haz.length*.35)));i++)g.haz.push({x:25+Math.random()*(W-50),y:55+Math.random()*(H-85),r:15+Math.random()*10,a:Math.random()*Math.PI*2,phase:Math.random()*Math.PI*2,orbit:20+Math.random()*55,speed:(.9+Math.random()*.8)*(g.n/100+1)})}if(c.modifier==='ICE FIELD')g.m.r=24;}}
function doubleScore(){if(typeof g==='undefined'||!g||g.over)return;const c=window.magnetChallengeActive;if(!c||c.level!==g.n||c.modifier!=='DOUBLE SCORE'){lastScore=g.score;return}if(g.score>lastScore)g.score+=g.score-lastScore;lastScore=g.score}
